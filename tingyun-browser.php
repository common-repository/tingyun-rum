<?php
/*
Plugin Name: Tingyun Browser Plugin for WordPress
Plugin URL: http://www.tingyun.com/tingyun_browser.html
Description: This Plugin will add Tingyun Browser RUM (Real User Monitoring) Agent on your WordPress website.
Version: 1.0.0
Author: groot@tingyun.com
Text Domain: tingyun-browser
*/

// Load language file(s)
load_plugin_textdomain('tingyun-browser', false, basename( dirname( __FILE__ ) ) . '/languages' );
$error_msg = '';

// Generate custom urls string
function tingyun_browser_custom_urls_string() {
	$ty_custum_urls = get_option('ty_browser_custom_urls');
	if (empty($ty_custum_urls))
		return '';
	$urls = split("\n", $ty_custum_urls);
	$result = '';
	foreach ($urls as $url) {
...
function tingyun_browser_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="' . admin_url('options-general.php?page=Tingyun_Browser_Config_Page') . '">' . __('Configuration', 'tingyun-browser') . '</a>';
...
// Admin menu function: Tingyun Browser RUM Agent configuration menu.
function tingyun_browser_rum_plugin_menu() {
	add_options_page(__('Tingyun Browser for WordPress Plugin', 'tingyun-browser'), __('Tingyun Browser', 'tingyun-browser'), 'manage_options', 'Tingyun_Browser_Config_Page', 'tingyun_browser_config_page');
}

// Check trace threshold
function tingyun_browser_check_trace_threshold($threshold) {
...
// Tingyun Browser admin configuration page.
function tingyun_browser_config_page() {
	 if (!current_user_can( 'manage_options'))  {
        wp_die(__('Sorry, you don\'t have sufficient permissions to access this configuration page.'));
    }
    wp_enqueue_style("tingyun_browser", plugin_dir_url("/", __FILE__) . trim(dirname(plugin_basename(__FILE__)), '/') . "/tingyun-browser.css");
?>
    <div id='container' class='wrap'>
    <img src="<?php echo plugin_dir_url("/", __FILE__) . trim(dirname(plugin_basename(__FILE__)), '/'); ?>/tingyun-logo.png" alt="Tingyun Logo" />

    <h1><?php _e('Tingyun Browser RUM (Real User Monitoring)', 'tingyun-browser') ?></h1>
<?php
    if (isset($_POST['submit'])) {
        update_option('ty_browser_application_id', trim($_POST['ty_browser_application_id']));
        update_option('ty_browser_application_key', trim($_POST['ty_browser_application_key']));
        update_option('ty_browser_custom_urls', stripslashes(trim($_POST['ty_browser_custom_urls'])));
        
        $trace_threshold = trim($_POST['ty_browser_trace_threshold']);
        if (tingyun_browser_check_trace_threshold($trace_threshold) && $trace_threshold >= 1000) 
        	update_option('ty_browser_trace_threshold', $trace_threshold);
        else
        	$error_msg = __('Trace threshold must be positive integer larger than 1000, in millisecond', 'tingyun-browser');
        
    }
    
    if (get_option('ty_browser_trace_threshold') == '')
    	update_option('ty_browser_trace_threshold', 7000);
?>
    <p><?php _e('Please fill your Tingyun Browser Application\'s ID and Key below.', 'tingyun-browser') ?> <?php _e('These can be found in your application configuration page on ', 'tingyun-browser') ?> <a href="https://report.tingyun.com/browser/" target="_blank"><?php _e('Tingyun Browser Console', 'tingyun-browser') ?></a></p>
    <p><?php _e('If you don\'t have a Tingyun account, you can sign in for free: ', 'tingyun-browser') ?> <a href="https://account.tingyun.com/reg/register?userFrom=wp_plugin#/" target="_blank" title="Sign in for Tingyun account"><?php _e('Sign in', 'tingyun-browser')?></a></p>
    <p><div class="ty_admin_errmsg"><?php echo $error_msg; ?></div></p>
    <form method="post" action="" class="ty_admin_form">
      <p> <?php _e('Application ID: ', 'tingyun-browser') ?>
        <input name="ty_browser_application_id" type="text" id="ty_browser_application_id" class="ty_admin_input" value="<?php echo get_option('ty_browser_application_id');?>" maxlength="11"/>
      </p>
      <p> <?php _e('Application Key: ', 'tingyun-browser') ?>
        <input name="ty_browser_application_key" type="text" id="ty_browser_application_key" class="ty_admin_input" value="<?php echo get_option('ty_browser_application_key');?>" maxlength="11"/>
      </p>
      <p> <?php _e('Trace threshold (ms): ', 'tingyun-browser') ?>
        <input name="ty_browser_trace_threshold" type="text" id="ty_browser_trace_threshold" class="ty_admin_input" value="<?php echo get_option('ty_browser_trace_threshold');?>" maxlength="11"/>
      </p>
      <p> <?php _e('Custom URLs: ', 'tingyun-browser') ?> <br>
        <textarea name="ty_browser_custom_urls" id="ty_browser_custom_urls" class="ty_admin_input" rows="5" cols="64"/><?php echo get_option('ty_browser_custom_urls');?></textarea>
      </p>
      <p>
        <input name="submit" type="submit" class="ty_admin_button"/>
      </p>
    </form>
    <p><a href="<?php echo get_bloginfo('wpurl');?>/wp-admin/plugins.php"><?php _e('Back to Plugins...', 'tingyun-browser'); ?></a></p>
</div>
<?php
} // End of tingyun_browser_config_page()

// uninstall clean up
function tingyun_browser_uninstall() {
    delete_option('ty_browser_application_id');
    delete_option('ty_browser_application_key');
    delete_option('ty_browser_trace_threshold');
    delete_option('ty_browser_custom_urls');
}


// Register actions and filters hooks
add_action('wp_head', 'tingyun_add_browser_rum_header', 0);
//if (current_user_can('manage_options'))
add_action('admin_menu', 'tingyun_browser_rum_plugin_menu');
add_filter('plugin_action_links', 'tingyun_browser_plugin_action_links', 10, 2);

// Register uninstall hook
if (function_exists('register_uninstall_hook')) {
    register_uninstall_hook(__FILE__, 'tingyun_browser_uninstall');
}
?>